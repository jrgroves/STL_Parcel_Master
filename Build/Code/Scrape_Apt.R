...
for(i in seq(1,nrow(b))){
  tryCatch({
webpage<-read_html(paste0(URL,"20K242260"))
...
tables<-webpage %>%
  html_nodes(xpath='//*[@id="divPropComData"]') %>%
  html_table(trim=TRUE,na.strings="NA") %>%
  as.data.frame()
  ...
  RESQFT<-NA
    ...
    tables<-tables[-c(1,2),-c(5,6,7,8)]
      temp1<-data.frame(rbind(tables[,1],tables[,2]))
        ...
        names(temp1)<-gsub(" ","_",names(temp1))
        temp1<-temp1[-1,]
      temp2<-data.frame(rbind(tables[,3],tables[,4]))
        ...
        temp2<-temp2[-1,]
    YEARBLT<-temp1$Year_Built
    GRADE<-temp1$`Grade?`
    RESQFT<-trimws(gsub("ft2","",temp2$Area_Under_Roof))
      RESQFT<-gsub(",","",RESQFT)
}
